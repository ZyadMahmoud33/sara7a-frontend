...
export default function PaymentModal({ plan, onClose, onConfirm }) {
  ...
  const paymentMethods = {
    ...
    instapay: {
      ...
      accountNumber: "instapay@sara7a.com He will open it in the future",
      accountName: "Sara7a App",
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className={cn('relative', 'bg-gradient-to-br', 'from-gray-900', 'to-black', 'shadow-2xl', 'border', 'border-white/10', 'rounded-2xl', 'w-full', 'max-w-lg', 'overflow-hidden')}
        >
          {/* Header */}
          <div className={cn('relative', 'bg-gradient-to-r', 'from-purple-600', 'to-pink-600', 'p-6')}>
            <button
              onClick={onClose}
              disabled={!!loadingMethod}
              className={cn('top-4', 'right-4', 'absolute', 'bg-white/10', 'hover:bg-white/20', 'p-1', 'rounded-lg', 'transition-colors')}
            >
              <X size={20} />
            </button>
            
            <div className="text-center">
              <div className={cn('inline-block', 'bg-white/20', 'mb-3', 'p-3', 'rounded-2xl')}>
                <Banknote size={32} />
              </div>
              <h2 className={cn('font-bold', 'text-2xl')}>Complete Payment</h2>
              <p className={cn('mt-1', 'text-white/80', 'text-sm')}>
          ...
          {/* Content */}
          <div className={cn('p-6', 'max-h-[70vh]', 'overflow-y-auto', 'custom-scrollbar')}>
            {/* Order Summary */}
            <div className={cn('bg-white/5', 'mb-6', 'p-4', 'border', 'border-white/10', 'rounded-xl')}>
              <h3 className={cn('flex', 'items-center', 'gap-2', 'mb-3', 'font-semibold')}>
                <Shield size={16} />
                Order Summary
              </h3>
              <div className={cn('space-y-2', 'text-sm')}>
                ...
                <div className={cn('mt-2', 'pt-2', 'border-white/10', 'border-t')}>
            ...
            <div className={cn('space-y-4', 'mb-6')}>
              <h3 className={cn('flex', 'items-center', 'gap-2', 'font-semibold')}>
                <CreditCard size={16} />
                ...
                >
                  <div className={cn('flex', 'items-center', 'gap-3')}>
                    ...
                    <div className="flex-1">
                      ...
                      <p className={cn('text-gray-400', 'text-xs')}>{method.description}</p>
                    </div>
                    {selectedMethod === key && (
            ...
            <AnimatePresence>
              {selectedMethod && selectedMethod !== "card" && (
                <motion.div
                  ...
                  className={cn('space-y-4', 'mb-6')}
                >
                  <div className={cn('bg-cyan-500/10', 'p-4', 'border', 'border-cyan-500/30', 'rounded-xl')}>
                    <div className={cn('flex', 'items-start', 'gap-3')}>
                      <AlertCircle size={20} className={cn('flex-shrink-0', 'mt-0.5', 'text-cyan-400')} />
                      <div className="flex-1">
                        <p className={cn('mb-2', 'font-semibold', 'text-cyan-400', 'text-sm')}>
                          Send payment to:
                        </p>
                        <div className="space-y-2">
                          <div className={cn('flex', 'justify-between', 'items-center')}>
                            <span className={cn('text-gray-300', 'text-sm')}>Account:</span>
                            <div className={cn('flex', 'items-center', 'gap-2')}>
                              ...
                              <button
                                ...
                                className={cn('hover:bg-white/10', 'p-1', 'rounded')}
                              >
                                <Copy size={14} />
                              </button>
                            </div>
                          ...
                          <div className={cn('flex', 'justify-between', 'items-center')}>
                            <span className={cn('text-gray-300', 'text-sm')}>Amount:</span>
                            <span className={cn('font-bold', 'text-green-400')}>
                              ${total.toFixed(2)}
                            </span>
                          </div>
                        </div>
                  ...
                  <div>
                    <label className={cn('block', 'mb-2', 'font-medium', 'text-sm')}>
                    ...
                    <input
                      ...
                      className={cn('bg-black/40', 'px-4', 'py-2', 'border', 'border-white/10', 'focus:border-purple-500', 'rounded-xl', 'focus:outline-none', 'w-full', 'transition-all', 'duration-200')}
                    />
                    <p className={cn('mt-1', 'text-gray-500', 'text-xs')}>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <label
                        ...
                        className={cn('flex', 'justify-center', 'items-center', 'gap-2', 'bg-white/5', 'hover:bg-white/10', 'px-4', 'py-3', 'border', 'border-white/10', 'rounded-xl', 'w-full', 'transition-all', 'duration-200', 'cursor-pointer')}
                      >
                        <Upload size={18} />
                  ...
                  {paymentMethods[selectedMethod].deepLink && countdown === null && (
                    <button
                      ...
                      className={cn('flex', 'justify-center', 'items-center', 'gap-2', 'bg-white/5', 'hover:bg-white/10', 'py-2', 'rounded-lg', 'w-full', 'transition-all', 'duration-200')}
                    >
                  ...
                  {/* Countdown Timer */}
                  {countdown !== null && (
                    <div className={cn('bg-blue-500/20', 'p-4', 'border', 'border-blue-500/30', 'rounded-lg', 'text-center')}>
                      <p className="text-sm">Redirecting to app in {countdown} seconds...</p>
                      <div className={cn('bg-white/10', 'mt-2', 'rounded-full', 'w-full', 'h-1', 'overflow-hidden')}>
                        <motion.div
                          initial={{ width: "100%" }}
                          animate={{ width: "0%" }}
                          transition={{ duration: 5, ease: "linear" }}
                          className={cn('bg-blue-500', 'rounded-full', 'h-full')}
                        />
                      </div>
                    </div>
                  )}
            ...
            {/* Security Note */}
            <div className={cn('bg-yellow-500/10', 'mt-6', 'p-3', 'border', 'border-yellow-500/30', 'rounded-lg')}>
              <div className={cn('flex', 'items-start', 'gap-2', 'text-yellow-400', 'text-xs')}>
                <Shield size={14} className={cn('flex-shrink-0', 'mt-0.5')} />
                <div>
                  <p className={cn('mb-1', 'font-semibold')}>Payment Security</p>
          ...
          {/* Footer */}
          <div className={cn('bg-black/30', 'p-4', 'border-white/10', 'border-t')}>
            <div className={cn('flex', 'justify-center', 'gap-4', 'text-gray-500', 'text-xs')}>
