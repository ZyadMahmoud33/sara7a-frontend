...
import { 
  ...
  Activity, Globe, Rocket, Link2, CopyCheck
} from "lucide-react";
...
const Starfield = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      gradient.addColorStop(0.6, 'rgba(236, 72, 153, 0.12)');
            ...
            {[...Array(150)].map((_, i) => (
              <motion.div
                ...
                style={{
                  ...
                  background: `radial-gradient(circle, ${['#a855f7', '#ec4899', '#06b6d4', '#10b981', '#f59e0b', '#3b82f6'][i % 6]}, transparent)`,
      ...
      <div className={cn('z-10', 'relative', 'mx-auto', 'p-4', 'md:p-6', 'max-w-7xl')}>
        ...
        <div className={cn('gap-5', 'grid', 'grid-cols-1', 'sm:grid-cols-2', 'lg:grid-cols-5', 'mb-8')}>
          {[
            ...
            { label: "Liked Messages", value: stats.liked, icon: Heart, color: "from-pink-600 to-rose-600", glow: "shadow-[0_0_30px_rgba(236,72,153,0.3)]", emoji: "❤️" },
            { label: "Account Status", value: user?.isPremium ? "Premium" : userPlan === "pro" ? "Pro" : "Free", icon: user?.isPremium ? Crown : userPlan === "pro" ? Zap : Shield, color: user?.isPremium ? "from-amber-500 to-orange-600" : userPlan === "pro" ? "from-cyan-500 to-blue-600" : "from-gray-600 to-gray-700", glow: "shadow-[0_0_30px_rgba(245,158,11,0.3)]", emoji: user?.isPremium ? "👑" : userPlan === "pro" ? "⚡" : "🛡️" },
            { label: "Coins", value: user?.coins || 0, icon: Coins, color: "from-yellow-500 to-orange-600", glow: "shadow-[0_0_30px_rgba(234,179,8,0.3)]", emoji: "💰" },
          ].map((stat, index) => (
            <motion.div
              key={stat.label}
        ...
        {/* Profile Section with Enhanced Copy Button */}
        <motion.div
            ...
            <div className="mt-6">
              ...
              <div className={cn('flex', 'gap-3')}>
                ...
                <motion.button 
                  ...
                  onClick={() => { 
                    if (navigator.share) { 
                      navigator.share({ title: 'My Anonymous Message Profile', text: 'Send me anonymous messages!', url: publicLink }); 
                    } else { 
                      copyLink(); 
                    } 
                  }} 
